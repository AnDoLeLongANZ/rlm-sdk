...
def get_client(
    ...
    elif backend == "vertex_anthropic":
        from rlm.clients.vertex_anthropic import VertexAnthropicClient

        return VertexAnthropicClient(**backend_kwargs)
    else:
        raise ValueError(
            f"Unknown backend: {backend}. Supported backends: ['openai', 'vllm', 'portkey', 'openrouter', 'litellm', 'anthropic', 'azure_openai', 'gemini', 'vercel', 'vertex_anthropic']"
        )
